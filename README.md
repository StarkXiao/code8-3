...
**参照物换算器只给区间，不下结论。** 空间导航里的「换算器」接收"半勺老抽""一小把虾皮"这类原话，用同一份换算引擎（`packages/shared/src/convert.ts`，前后端共用、带单元测试）按已登记餐具折成克数，并给容错区间。区间不是拍脑袋：基础误差（登记本身约 ±12%）之上，分数份（半勺/小半）、没说平满、满勺、大小号、ml 按 1g/ml 估算、手抓等每一层"靠眼睛"的不确定性都会累加（钳制在 ±15%–50%）。"满勺约 12g"会优先从参照物备注里读实测倍率，而不是写死。匹配不到餐具时宁可提示"去登记"也不瞎猜碗、盆的大小；只有茶匙/汤匙（国标 5ml/15ml）与"一把/一撮"给经验估值并标为暂定/推算。换算结果永远不是"已确认"，落笔仍需人确认。
