...
**参照物换算器。** 成员页的参照物卡片下方直接输入"半勺""一小把""两大勺""满勺"，按已登记的餐具折算克数并给出容错区间：勺/碗类默认 ±15%，手感（把/捏/撮）默认 ±30%~35%，半份、小把、堆尖各再叠加 5%。登记了多把勺且口述分不清是哪把时，会列出候选让你选；量的是 ml 就报 ml，不擅自按密度换算成克。换算口径在 `packages/shared/src/reference.ts`，前后端共用一份，并附 23 个单测（`reference.test.ts`）。
...
```bash
npm run test        # 后端 80+ 个测试（含参照物换算器 23 个单测）
